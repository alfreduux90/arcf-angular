...
export class UserEffects {
  ...
  signUpEmail: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ofType(fromActions.Types.SIGNIN_UP_EMAIL),
  ...
  this.actions.pipe(
    ...
    switchMap((userData) =>
      this.httpClient.post<UserResponse>(`${environment.url}account/login/`, userData)
        .pipe(
          tap((response: UserResponse) => {
            localStorage.setItem('token', response.token.access);
            this.router.navigate(['/']);
  ...
  this.actions.pipe(
    ...
    switchMap( token => {
      if (token){
        ...
        .pipe(
          tap((response: UserResponse) => {
            console.log('data del usuario que viene desde server', response);
          }),
